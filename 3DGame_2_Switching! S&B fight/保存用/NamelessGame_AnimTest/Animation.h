...
public:
	...
	virtual void Init();
	virtual void Update(const int& model);
	virtual void Draw();
...
public:
	...
	void SetAnim(const int& model,int animNo, bool isLoop, bool isForceChange);
	...
	void ChangeAnim(const int& model,int animNo, bool isLoop, bool isForceChange, bool isChangeFrame);

	// 現在のアニメーションが終了しているかどうかを取得する(Loopアニメの場合は取得できない = falseを返す)
	bool IsAnimEnd(const int& model);
