...
void Animation::Update(int model)
{
	// モデルのアップデート
	m_animSpeed++;
...
void Animation::SetAnim(int model,int animNo, bool isLoop, bool isForceChange)
{	
	// アニメーションを再生させるか
	if (!isForceChange)
	{
...
void Animation::ChangeAnim(int model, int animNo, bool isLoop, bool isForceChange, bool isChangeFrame)
{
	// アニメーションを再生させるか
	if (!isForceChange)
	{
...
bool Animation::IsAnimEnd(int model)
{
...
void Animation::UpdateAnimBlendRate(int model)
{
	...
	// アニメーションのブレンド率を設定する
	MV1SetAttachAnimBlendRate(model, m_prev.animNo, kAnimBlendMax - m_animBlendRate);
	// アニメーションのブレンド率を設定する
	MV1SetAttachAnimBlendRate(model, m_current.animNo, m_animBlendRate);
